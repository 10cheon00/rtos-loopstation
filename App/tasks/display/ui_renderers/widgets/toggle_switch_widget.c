...
void UiWidget_DrawToggleSwitchWidget(u8g2_t *u8g2, uint8_t x, uint8_t y, Parameter *parameter)
{
    ...
    u8g2_DrawRBox(u8g2, x, y, TOGGLE_SWITCH_RBOX_WIDTH, TOGGLE_SWITCH_RBOX_HEIGHT, 2);
    u8g2_SetDrawColor(u8g2, 0);
    indicator_cx = x + PADDING;
    if (parameter->current == parameter->min) {
        indicator_cy =
            y + TOGGLE_SWITCH_RBOX_HEIGHT - (PADDING + TOGGLE_SWITCH_INDICATOR_HEIGHT);
    } else {
