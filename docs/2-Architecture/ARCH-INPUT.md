...
요구사항 문서는 사용자가 어떤 조작을 시스템 입력으로 제공할 수 있어야 하는지를 설명하고, 이 문서는 그 입력을 감지, 정규화, 전달, 해석하기 위한 내부 구조를 설명한다.
...
`REQ-INPUT-001`은 제어부의 좌, 우, Enter, Exit 조작을 디스플레이 패널 조작 입력으로 받을 수 있어야 한다는 요구사항이다.
...
`REQ-INPUT-002`는 FX부의 FX 조작을 FX 활성화 입력으로 받을 수 있어야 한다는 요구사항이다.
...
`REQ-INPUT-004`는 트랙부의 아날로그 조작을 트랙 볼륨 변경 입력으로 받을 수 있어야 한다는 요구사항이다.
...
`REQ-INPUT-005`는 로터리 조작의 회전 방향을 입력으로 받을 수 있어야 한다는 요구사항이다.
...
| `FEAT-encoder-value-input.md` | 엔코더 값 변경 해석 | 회전 event를 현재 선택 항목의 값 변경으로 해석한다. |

### 3.6 REQ-INPUT-006 설계

`REQ-INPUT-006`은 로터리 조작의 누름을 입력으로 받을 수 있어야 한다는 요구사항이다.
...
| `FEAT-track-volume-input.md` | 아날로그 입력을 트랙 볼륨 변경으로 해석한다. | `CONTROL_POT_CHANGE` | track volume state |
| `FEAT-encoder-value-input.md` | 엔코더 회전을 현재 선택 항목의 값 변경으로 해석한다. | `CONTROL_ENCODER_ROTATE` | selected value change |
| `FEAT-encoder-push-input.md` | 엔코더 push를 선택 또는 modifier 입력으로 해석한다. | `CONTROL_BUTTON` | selection/modifier state |
