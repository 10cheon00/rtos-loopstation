...
이 문서는 루프스테이션 요구사항 중 `REQ-STATE-DISPLAY` 항목을 만족시키기 위한 디스플레이 패널 상태 머신 구조를 정의한다.
...
| 항목 | 내용 |
| --- | --- |
| 대상 요구사항 | `REQ-STATE-DISPLAY-001` ~ `REQ-STATE-DISPLAY-013` |
| 포함 범위 | 초기 홈 패널, 상위 패널 좌우 탐색, Enter 하위 패널 진입, Exit 상위 패널 복귀, IFX/TFX 설정 패널 직접 진입, 패널 전환 render 요청 |
| 연관 설계 | 상태 머신 공통 구조, 사용자 입력, 표시 및 피드백, 시스템 상태 머신, 오디오 효과 |
| 제외 범위 | LCD widget 배치, u8g2 drawing API, 패널별 표시값 선정, FX 활성화 및 파라미터 변경, 물리 버튼 debounce |

## 2. 관련 요구사항

| 요구사항 ID | 요구사항 요약 | 이 문서의 설계 관점 |
| --- | --- | --- |
| `REQ-STATE-DISPLAY-001` | 디스플레이 패널 상태 머신을 홈 패널에서 시작한다. | 초기 상태를 `HOME`으로 등록하고 최초 render를 요청한다. |
| `REQ-STATE-DISPLAY-002` | 설정 패널에서 우 조작 시 홈 패널로 전환한다. | 상위 패널 우측 탐색표에 `SETTING -> HOME`을 둔다. |
| `REQ-STATE-DISPLAY-003` | 홈 패널에서 좌 조작 시 설정 패널로 전환한다. | 상위 패널 좌측 탐색표에 `HOME -> SETTING`을 둔다. |
| `REQ-STATE-DISPLAY-004` | 홈 패널에서 우 조작 시 FX 패널로 전환한다. | 상위 패널 우측 탐색표에 `HOME -> FX`를 둔다. |
| `REQ-STATE-DISPLAY-005` | FX 패널에서 좌 조작 시 홈 패널로 전환한다. | 상위 패널 좌측 탐색표에 `FX -> HOME`을 둔다. |
| `REQ-STATE-DISPLAY-006` | FX 패널에서 우 조작 시 트랙 패널로 전환한다. | 상위 패널 우측 탐색표에 `FX -> TRACK`을 둔다. |
| `REQ-STATE-DISPLAY-007` | 트랙 패널에서 좌 조작 시 FX 패널로 전환한다. | 상위 패널 좌측 탐색표에 `TRACK -> FX`를 둔다. |
| `REQ-STATE-DISPLAY-008` | 설정 패널에서 선택 항목에 해당하는 하위 패널로 진입한다. | 선택 context를 조회해 `SYSTEM_SETTING` 또는 `HARDWARE_DIAGNOSTIC`으로 전이한다. |
| `REQ-STATE-DISPLAY-009` | FX 패널에서 선택한 FX 설정 패널로 진입한다. | 선택 FX bus에 따라 `IFX_SETTING` 또는 `TFX_SETTING`으로 전이한다. |
| `REQ-STATE-DISPLAY-010` | 트랙 패널에서 트랙 설정 패널로 진입한다. | Enter 이벤트를 `TRACK -> TRACK_SETTING` 전이로 매핑한다. |
| `REQ-STATE-DISPLAY-011` | 하위 패널에서 Exit 조작 시 상위 패널로 복귀한다. | 각 하위 패널에 고정된 parent panel을 두고 Exit 전이에 사용한다. |
| `REQ-STATE-DISPLAY-012` | IFX 조작 시 IFX 설정 패널로 전환한다. | 현재 패널과 관계없이 IFX 직접 진입 전이를 적용한다. |
| `REQ-STATE-DISPLAY-013` | TFX 조작 시 TFX 설정 패널로 전환한다. | 현재 패널과 관계없이 TFX 직접 진입 전이를 적용한다. |

## 3. 요구사항별 설계

### 3.1 REQ-STATE-DISPLAY-001 설계

디스플레이 패널 상태 머신은 시스템이 정상 실행을 시작할 때 `HOME` 상태로 등록된다.
최초 `HOME.on_enter` 완료 후 상태 관리 구조는 홈 패널을 출력하기 위한 `UI_STATE_RENDER` 요청을 표시 구조로 전달한다.

| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-STATE-DISPLAY-001` | 패널 상태 모델 | 현재 선택된 패널을 canonical state로 유지한다. | 상위 패널과 하위 패널을 정적 상태로 등록한다. |
| `ARCH-STATE-DISPLAY-002` | 패널 context | 선택 항목과 대상 정보를 상태와 분리한다. | `selected_item`, `selected_track`, `selected_fx_bus`를 context에 보관한다. |
| `ARCH-STATE-DISPLAY-003` | 초기 패널 상태 | 최초 표시 패널을 고정한다. | 초기 상태 참조를 `HOME`으로 등록한다. |
| `ARCH-STATE-DISPLAY-004` | 최초 패널 render | canonical panel state와 실제 화면을 일치시킨다. | `HOME.on_enter` 완료 후 `UI_STATE_RENDER(HOME)`을 생성한다. |

```mermaid
sequenceDiagram
    participant System as 시스템 상태 머신
    participant StateTask as 상태 관리 태스크
    participant Panel as 디스플레이 패널 상태 머신
    participant Display as 표시 구조

    System->>StateTask: 일반 상태 머신 활성화
    StateTask->>Panel: 초기 상태 HOME 등록
    StateTask->>Panel: HOME.on_enter
    Panel-->>StateTask: UI_STATE_RENDER(HOME)
    StateTask->>Display: UI_STATE_RENDER(HOME)
```

### 3.2 REQ-STATE-DISPLAY-002 ~ REQ-STATE-DISPLAY-007 설계
...
| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-STATE-DISPLAY-005` | 상위 패널 순서 | 좌우 탐색의 인접 관계를 정의한다. | `SETTING`, `HOME`, `FX`, `TRACK` 순서의 비순환 탐색표를 사용한다. |
| `ARCH-STATE-DISPLAY-006` | 좌 조작 전이 | 현재 패널의 왼쪽 인접 패널을 선택한다. | `HOME -> SETTING`, `FX -> HOME`, `TRACK -> FX`를 정의한다. |
| `ARCH-STATE-DISPLAY-007` | 우 조작 전이 | 현재 패널의 오른쪽 인접 패널을 선택한다. | `SETTING -> HOME`, `HOME -> FX`, `FX -> TRACK`을 정의한다. |
| `ARCH-STATE-DISPLAY-008` | 탐색 경계 | 상위 패널 목록의 양 끝에서 범위를 벗어나지 않게 한다. | 목적지가 없는 방향 조작은 `HANDLED`로 처리하고 현재 상태를 유지한다. |
| `ARCH-STATE-DISPLAY-009` | 상위 패널 render | 상태 전이 결과를 LCD에 반영한다. | 전이한 패널의 `on_enter`가 `UI_STATE_RENDER` 요청을 생성한다. |
...
sequenceDiagram
    ...
    Panel-->>StateTask: UI_STATE_RENDER(next_panel, context)
    StateTask->>Display: UI_STATE_RENDER(next_panel, context)
```

### 3.3 REQ-STATE-DISPLAY-008 ~ REQ-STATE-DISPLAY-010 설계
...
| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-STATE-DISPLAY-010` | Enter 대상 선택 | 상위 패널의 선택 항목을 하위 패널로 변환한다. | 현재 패널과 context를 Enter 전이표에 적용한다. |
| `ARCH-STATE-DISPLAY-011` | 설정 하위 패널 | 설정 항목에 대응하는 panel state를 선택한다. | 시스템 설정은 `SYSTEM_SETTING`, 하드웨어 점검은 `HARDWARE_DIAGNOSTIC`으로 전이한다. |
| `ARCH-STATE-DISPLAY-012` | FX 하위 패널 | 선택된 FX bus에 대응하는 panel state를 선택한다. | IFX는 `IFX_SETTING`, TFX는 `TFX_SETTING`으로 전이한다. |
| `ARCH-STATE-DISPLAY-013` | 트랙 하위 패널 | 선택 트랙의 설정 화면으로 진입한다. | `TRACK -> TRACK_SETTING`으로 전이하고 `selected_track`을 유지한다. |
| `ARCH-STATE-DISPLAY-014` | 하위 패널 render | 선택 대상과 함께 하위 패널을 출력한다. | `UI_STATE_RENDER`에 panel id와 현재 선택 context를 포함한다. |
...
| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-STATE-DISPLAY-015` | parent panel mapping | 하위 패널별 복귀 목적지를 정의한다. | 각 하위 panel state 등록 정보에 parent panel을 둔다. |
| `ARCH-STATE-DISPLAY-016` | Exit 전이 | 하위 패널에서 상위 패널로 돌아간다. | Exit 처리 결과로 `TRANSITION(parent_panel)`을 반환한다. |
| `ARCH-STATE-DISPLAY-017` | 선택 context 유지 | 복귀 전후의 사용자 선택을 보존한다. | Exit 전이에서는 parent panel의 선택 context를 초기화하지 않는다. |
| `ARCH-STATE-DISPLAY-018` | 복귀 panel render | 복귀한 상위 패널을 화면에 반영한다. | parent panel의 `on_enter`가 기존 context로 `UI_STATE_RENDER`를 생성한다. |
...
### 3.5 REQ-STATE-DISPLAY-012 및 REQ-STATE-DISPLAY-013 설계
...
| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-STATE-DISPLAY-019` | FX 직접 진입 이벤트 | 현재 패널과 관계없이 FX 설정 패널 이동을 요청한다. | IFX와 TFX 조작 이벤트를 공통 패널 전이표보다 먼저 검사한다. |
| `ARCH-STATE-DISPLAY-020` | IFX 직접 전이 | IFX 조작의 목적 패널을 고정한다. | 현재 상태에서 `IFX_SETTING`으로 전이하고 `selected_fx_bus=IFX`로 갱신한다. |
| `ARCH-STATE-DISPLAY-021` | TFX 직접 전이 | TFX 조작의 목적 패널을 고정한다. | 현재 상태에서 `TFX_SETTING`으로 전이하고 `selected_fx_bus=TFX`로 갱신한다. |
| `ARCH-STATE-DISPLAY-022` | 직접 진입 parent | 직접 진입 이후 Exit 목적지를 일관되게 한다. | 두 FX 설정 패널의 parent panel을 `FX`로 유지한다. |
| `ARCH-STATE-DISPLAY-023` | 직접 진입 render | 선택한 FX 설정 화면을 즉시 표시한다. | 전이 완료 후 FX bus context를 포함한 `UI_STATE_RENDER`를 생성한다. |
...
sequenceDiagram
    ...
    Panel-->>StateTask: UI_STATE_RENDER(fx_setting, fx_bus)
    StateTask->>Display: UI_STATE_RENDER(fx_setting, fx_bus)
```
...
| 책임 | 담당 |
| --- | --- |
| 입력 event를 display state event로 분류 | 상태 관리 구조의 이벤트 분류기 |
| 현재 panel state와 선택 context 유지 | 디스플레이 패널 상태 머신 |
| 전이 유효성 검사와 생명주기 호출 | `ARCH-STATE.md`의 공통 상태 관리 구조 |
| 전이 완료 후 `UI_STATE_RENDER` 생성 | 디스플레이 패널 상태 머신 |
| render command queue 전송 | 상태 관리 구조 |
...
| 이름 | 방향 | 용도 |
| --- | --- | --- |
| 좌/우 control event | 사용자 입력 구조 -> 디스플레이 패널 상태 머신 | 상위 패널 탐색 |
| Enter control event | 사용자 입력 구조 -> 디스플레이 패널 상태 머신 | 선택된 하위 패널 진입 |
| Exit control event | 사용자 입력 구조 -> 디스플레이 패널 상태 머신 | parent panel 복귀 |
| IFX/TFX direct event | 사용자 입력 구조 -> 디스플레이 패널 상태 머신 | FX 설정 패널 직접 진입 |
| `UI_STATE_RENDER` | 디스플레이 패널 상태 머신 -> 표시 구조 | panel id와 선택 context에 따른 화면 전환 |
...
| 오류 panel | 시스템 오류 표시를 display state로 둘지 표시 command만 사용할지 정해야 한다. | 시스템 오류와 panel state 관계 |
