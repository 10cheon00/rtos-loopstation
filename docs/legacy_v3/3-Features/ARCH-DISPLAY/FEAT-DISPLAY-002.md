...
| 주요 목적 | UI render command를 누락 없이 순서대로 표시 구조에 전달한다. |
...
| --- | --- | --- |
| `ARCH-DISPLAY-006` | display command queue | 패널 전환과 파라미터 변경에 따른 표시 명령을 순서대로 처리한다. |
...
| 입력 | 설명 |
| --- | --- |
| `UI_STATE_RENDER` command | StateTask가 이벤트 처리 완료 후 생성한 panel ID와 바인딩 파라미터 command |
