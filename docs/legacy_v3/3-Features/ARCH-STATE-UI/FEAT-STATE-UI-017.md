...
| 항목 | 내용 |
| --- | --- |
| 기능 ID | `FEAT-STATE-UI-017` |
| 상위 설계 문서 | `ARCH-STATE-UI.md` |
| 관련 설계 항목 | `ARCH-STATE-UI-017` |
...
| --- | --- | --- |
| `ARCH-STATE-UI-017` | 선택 context 유지 | Exit 전이에서는 parent panel의 선택 context를 초기화하지 않는다. |
...
| 출력 | 설명 |
| --- | --- |
| 유지된 UI context | 기존 selected item, track과 FX bus |
...
| 설계 추적 | 구현과 시험이 `ARCH-STATE-UI-017`의 설계 결정을 만족하는지 검토한다. |
