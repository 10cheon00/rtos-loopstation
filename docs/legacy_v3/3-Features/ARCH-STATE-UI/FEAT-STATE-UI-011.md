...
| 항목 | 내용 |
| --- | --- |
| 기능 ID | `FEAT-STATE-UI-011` |
| 상위 설계 문서 | `ARCH-STATE-UI.md` |
| 관련 설계 항목 | `ARCH-STATE-UI-011` |
| 관련 요구사항 | `REQ-STATE-UI-008` |
...
| --- | --- | --- |
| `ARCH-STATE-UI-011` | 설정 하위 패널 | 시스템 설정은 `SYSTEM_SETTING`, 하드웨어 점검은 `HARDWARE_DIAGNOSTIC`으로 전이한다. |
...
| 설계 추적 | 구현과 시험이 `ARCH-STATE-UI-011`의 설계 결정을 만족하는지 검토한다. |
