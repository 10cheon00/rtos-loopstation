...
| 항목 | 내용 |
| --- | --- |
| 기능 ID | `FEAT-STATE-UI-013` |
| 상위 설계 문서 | `ARCH-STATE-UI.md` |
| 관련 설계 항목 | `ARCH-STATE-UI-013` |
| 관련 요구사항 | `REQ-STATE-UI-010` |
...
| --- | --- | --- |
| `ARCH-STATE-UI-013` | 트랙 하위 패널 | `TRACK -> TRACK_SETTING`으로 전이하고 `selected_track`을 유지한다. |
...
| 설계 추적 | 구현과 시험이 `ARCH-STATE-UI-013`의 설계 결정을 만족하는지 검토한다. |
