...
| 항목 | 내용 |
| --- | --- |
| 기능 ID | `FEAT-STATE-UI-021` |
| 상위 설계 문서 | `ARCH-STATE-UI.md` |
| 관련 설계 항목 | `ARCH-STATE-UI-021` |
| 관련 요구사항 | `REQ-STATE-UI-013` |
...
| --- | --- | --- |
| `ARCH-STATE-UI-021` | TFX 직접 전이 | 현재 상태에서 `TFX_SETTING`으로 전이하고 `selected_fx_bus=TFX`로 갱신한다. |
...
| 설계 추적 | 구현과 시험이 `ARCH-STATE-UI-021`의 설계 결정을 만족하는지 검토한다. |
