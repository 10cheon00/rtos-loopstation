...
change_history:
  ...
  - date: 2026-07-18
    version: 1.0.0
    summary: 트랙 요구사항과 설계 식별자를 상태 머신 분류 체계에 맞게 변경함
---

# 트랙 상태 머신 아키텍처

이 문서는 루프스테이션 요구사항 중 `REQ-STATE-TRACK` 항목을 만족시키기 위한 트랙 상태 전이 구조를 정의한다.
...
| 항목 | 내용 |
| --- | --- |
| 대상 요구사항 | `REQ-STATE-TRACK-001` ~ `REQ-STATE-TRACK-012` |
...
| --- | --- | --- |
| `REQ-STATE-TRACK-001` | 전원 인가 이후 각 트랙을 초기화되지 않은 상태로 둔다. | 부팅 시 canonical track state를 `NOT_INIT`으로 생성한다. |
| `REQ-STATE-TRACK-002` | 초기화되지 않은 트랙을 초기화할 수 있어야 한다. | 상태 관리 구조가 저장 구조에 reset/open 준비를 요청한다. |
| `REQ-STATE-TRACK-003` | 초기화 완료 후 대기 상태로 전환한다. | 초기화 성공 결과를 받아 `IDLE`로 전이한다. |
| `REQ-STATE-TRACK-004` | 대기 상태에서 녹음 시작 입력 시 녹음 상태로 전환한다. | `IDLE -> RECORDING` 전이와 `TRACK_RECORD_START` command를 연결한다. |
| `REQ-STATE-TRACK-005` | 녹음 상태에서 정지 입력 시 정지 상태로 전환한다. | 녹음 종료 목표 상태를 `STOPPED`로 저장한다. |
| `REQ-STATE-TRACK-006` | 녹음 완료 입력 시 재생 상태로 전환한다. | `AUDIO_RECORD_DONE` 수신 후 저장된 목표 상태가 `PLAYING`이면 재생을 시작한다. |
| `REQ-STATE-TRACK-007` | 재생 상태에서 정지 입력 시 정지 상태로 전환한다. | `TRACK_PLAY_STOP` command와 `PLAYING -> STOPPED` 전이를 연결한다. |
| `REQ-STATE-TRACK-008` | 재생 상태에서 오버더빙 시작 입력 시 오버더빙 상태로 전환한다. | `TRACK_OVERDUB_START` command와 `PLAYING -> OVERDUBBING` 전이를 연결한다. |
| `REQ-STATE-TRACK-009` | 오버더빙 상태에서 정지 입력 시 정지 상태로 전환한다. | `TRACK_OVERDUB_STOP` command와 `OVERDUBBING -> STOPPED` 전이를 연결한다. |
| `REQ-STATE-TRACK-010` | 오버더빙 완료 입력 시 재생 상태로 전환한다. | `TRACK_OVERDUB_FINISH` command와 `OVERDUBBING -> PLAYING` 전이를 연결한다. |
| `REQ-STATE-TRACK-011` | 정지 상태에서 재생 시작 입력 시 재생 상태로 전환한다. | `TRACK_PLAY_START` command와 `STOPPED -> PLAYING` 전이를 연결한다. |
| `REQ-STATE-TRACK-012` | 정지 상태에서 리셋 입력 시 대기 상태로 전환한다. | 저장 구조의 트랙 초기화 결과를 받아 `IDLE`로 전이한다. |

## 3. 요구사항별 설계

### 3.1 REQ-STATE-TRACK-001 ~ REQ-STATE-TRACK-003 설계
...
| --- | --- | --- | --- |
| `ARCH-STATE-TRACK-001` | 트랙 상태 모델 | 트랙별 canonical state를 저장한다. | 상태 관리 구조가 소유한다. |
| `ARCH-STATE-TRACK-002` | 초기화 요청 | 트랙 파일 또는 metadata를 사용할 수 있는 상태로 만든다. | 저장 구조에 `TRACK_FILE_RESET` 또는 준비 요청을 보낸다. |
| `ARCH-STATE-TRACK-003` | 초기화 결과 처리 | 성공/실패에 따라 상태를 갱신한다. | 성공 시 `IDLE`, 실패 시 오류 표시 경로로 보낸다. |
| `ARCH-STATE-TRACK-004` | 상태 표시 | 초기화 결과를 사용자에게 보여준다. | `TRACK_STATE_SNAPSHOT_RENDER`를 표시 구조로 보낸다. |
...
### 3.2 REQ-STATE-TRACK-004 ~ REQ-STATE-TRACK-006 설계
...
| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-STATE-TRACK-005` | 녹음 시작 판단 | `IDLE` 트랙에서 녹음 입력을 해석한다. | 상태 관리 구조가 `TRACK_RECORD_START`를 오디오 처리 구조로 보낸다. |
| `ARCH-STATE-TRACK-006` | 녹음 종료 목표 상태 | 종료 입력의 의도를 저장한다. | 녹음/재생 버튼이면 `PLAYING`, 정지 버튼이면 `STOPPED`를 로컬에 보관한다. |
| `ARCH-STATE-TRACK-007` | 녹음 완료 보고 | 실제 저장 완료를 상태 전이 조건으로 사용한다. | `AUDIO_RECORD_DONE` 수신 후 목표 상태로 전이한다. |
| `ARCH-STATE-TRACK-008` | 표시 갱신 | 상태 변화마다 전체 트랙 snapshot을 보낸다. | 표시 구조는 snapshot을 기준으로 LED/LCD를 갱신한다. |
...
### 3.3 REQ-STATE-TRACK-007 및 REQ-STATE-TRACK-011 설계

재생과 정지 요구사항은 저장된 트랙이 있는 상태에서 `PLAYING`과 `STOPPED` 사이를 오가는 흐름을 정의한다.

| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-STATE-TRACK-009` | 재생 시작 판단 | `STOPPED` 트랙에서 재생 입력을 해석한다. | `TRACK_PLAY_START`를 오디오 처리 구조로 보낸다. |
| `ARCH-STATE-TRACK-010` | 재생 정지 판단 | `PLAYING` 트랙에서 정지 입력을 해석한다. | `TRACK_PLAY_STOP`을 오디오 처리 구조로 보낸다. |
| `ARCH-STATE-TRACK-011` | 재생 위치 관리 | 실제 loop position은 오디오 처리 구조가 관리한다. | 상태 관리 구조는 canonical state만 갱신한다. |
...
### 3.4 REQ-STATE-TRACK-008 ~ REQ-STATE-TRACK-010 설계
...
| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-STATE-TRACK-012` | 오버더빙 시작 | `PLAYING` 트랙에서 녹음/재생 입력을 해석한다. | `TRACK_OVERDUB_START`를 보낸 뒤 `OVERDUBBING`으로 전이한다. |
| `ARCH-STATE-TRACK-013` | 오버더빙 완료 | 오버더빙 후 재생으로 돌아간다. | `TRACK_OVERDUB_FINISH`를 보내고 `PLAYING`으로 전이한다. |
| `ARCH-STATE-TRACK-014` | 오버더빙 정지 | 오버더빙 후 정지한다. | `TRACK_OVERDUB_STOP`을 보내고 `STOPPED`로 전이한다. |
| `ARCH-STATE-TRACK-015` | 저장 정책 | 현재는 destructive overwrite를 전제로 한다. | undo/redo는 저장 설계의 미정 사항으로 둔다. |
...
| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-STATE-TRACK-016` | 리셋 조건 확인 | `STOPPED` 상태에서만 리셋을 허용한다. | 상태 관리 구조가 현재 track state를 검사한다. |
| `ARCH-STATE-TRACK-017` | 파일 초기화 요청 | 트랙 audio data와 metadata를 초기화한다. | 저장 구조에 `TRACK_FILE_RESET`을 보낸다. |
| `ARCH-STATE-TRACK-018` | 리셋 결과 처리 | 성공 시 `IDLE`로 전이한다. | 실패 시 오류 표시 경로로 보낸다. |
