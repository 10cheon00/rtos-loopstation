---
title: 표시 및 피드백 아키텍처
version: 1.1.0
change_history:
  - date: 2026-07-11
    version: 0.1.0
    summary: 표시 및 피드백 요구사항별 설계와 공통 표시 메시지 구조를 작성함
  - date: 2026-07-11
    version: 0.2.0
    summary: 선택된 패널에 필요한 표시값만 전달하도록 표시값 갱신 설계를 수정함
  - date: 2026-07-12
    version: 0.3.0
    summary: REQ-DISPLAY-002 구현을 위한 단일 기능 문서 참조를 추가함
  - date: 2026-07-12
    version: 0.4.0
    summary: 설계 항목별 고유 식별자를 추가함
  - date: 2026-07-26
    version: 1.0.0
    summary: UI_STATE_RENDER를 panel ID와 모든 바인딩 파라미터를 포함하는 자기완결형 command로 변경함
  - date: 2026-07-28
    version: 1.0.1
    summary: 문서의 설계 및 설명 내용을 갱신함
  - date: 2026-07-28
    version: 1.1.0
    summary: 파라미터 변경 시 현재 패널 전체를 다시 그리는 설계와 기능 명세 대상을 반영함
---

# 표시 및 피드백 아키텍처

이 문서는 루프스테이션 요구사항 중 `REQ-DISPLAY` 항목을 만족시키기 위한 표시 및 피드백 구조를 정의한다.
요구사항 문서는 시스템이 사용자에게 어떤 상태를 보여야 하는지를 설명하고, 이 문서는 상태 변경을 LCD와 LED 표시로 반영하기 위한 내부 구조를 설명한다.

## 1. 설계 범위

| 항목 | 내용 |
| --- | --- |
| 대상 요구사항 | `REQ-DISPLAY-001`, `REQ-DISPLAY-002`, `REQ-DISPLAY-003`, `REQ-DISPLAY-004` |
| 포함 범위 | FX LED 표시, LCD 패널 전환, 표시값 갱신, 트랙 상태 LED 표시, 표시 메시지 queue/mailbox 구조 |
| 연관 설계 | 사용자 입력, 트랙 생명주기, 오디오 효과, 실시간 동작 |
| 제외 범위 | 사용자 입력 해석, FX 알고리즘, 트랙 상태 전이 판단, LCD 드라이버 초기화 코드 세부 구현 |

## 2. 관련 요구사항

| 요구사항 ID | 요구사항 요약 | 이 문서의 설계 관점 |
| --- | --- | --- |
| `REQ-DISPLAY-001` | FX 활성화 상태에 따라 표시등을 켜거나 끈다. | FX 상태 snapshot을 받아 IFX/TFX LED 상태를 갱신한다. |
| `REQ-DISPLAY-002` | 선택된 디스플레이 패널을 화면에 출력한다. | 상태 관리 구조가 현재 UI 상태의 panel ID와 그 패널에 바인딩된 모든 파라미터를 표시 command로 전달한다. |
| `REQ-DISPLAY-003` | 선택된 패널에 표시될 값이 바뀌면 화면의 값을 갱신한다. | 상태 관리 구조가 현재 패널의 바인딩 파라미터를 전달하면 표시 구조가 패널 전체를 다시 그린다. |
| `REQ-DISPLAY-004` | 각 트랙 상태를 표시등의 색으로 표현한다. | 모든 트랙의 상태 snapshot을 받아 트랙 LED 색을 일관되게 갱신한다. |

## 3. 요구사항별 설계

### 3.1 REQ-DISPLAY-001 설계

`REQ-DISPLAY-001`은 FX 조작 결과가 FX 활성화 표시등에 즉시 반영되어야 한다는 요구사항이다.
FX 활성화 여부는 상태 관리 구조가 canonical state로 소유하고, 표시 구조는 전달받은 FX 상태 snapshot을 기준으로 LED와 LCD 표시를 갱신한다.

#### 3.1.1 필요 아키텍처

| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-DISPLAY-001` | FX 상태 snapshot | IFX/TFX 선택 FX와 활성화 상태를 한 번에 표현한다. | `FX_STATE_SNAPSHOT_RENDER`를 사용한다. |
| `ARCH-DISPLAY-002` | FX 상태 mailbox | 최신 FX 상태만 유지한다. | `fx_state_mailbox`를 overwritable queue로 둔다. |
| `ARCH-DISPLAY-003` | FX LED renderer | 활성화 여부를 LED on/off로 변환한다. | 표시 구조는 상태 판단 없이 payload를 표시한다. |
| `ARCH-DISPLAY-004` | FX LCD renderer | 현재 FX 패널에 표시되는 활성화 상태를 갱신한다. | 표시 중인 패널이 FX 관련이면 LCD 값도 갱신한다. |

#### 3.1.2 구조 다이어그램

```mermaid
flowchart LR
    State["상태 관리 구조"]
    Mailbox["fx_state_mailbox"]
    Display["표시 구조"]
    Led["FX LED"]
    Lcd["LCD FX 영역"]

    State -->|FX_STATE_SNAPSHOT_RENDER| Mailbox --> Display
    Display --> Led
    Display --> Lcd
```

#### 3.1.3 동작 시나리오

```mermaid
sequenceDiagram
    participant State as 상태 관리 구조
    participant Display as 표시 구조
    participant Led as FX LED
    participant Lcd as LCD

    State->>Display: FX_STATE_SNAPSHOT_RENDER(IFX, TFX)
    Display->>Display: IFX/TFX enabled 값 확인
    Display->>Led: LED on/off 갱신
    opt 현재 패널이 FX 패널
        Display->>Lcd: FX 활성화 표시 갱신
    end
```

#### 3.1.4 개발해야 할 기능

| 기능 문서 | 기능 | 목적 |
| --- | --- | --- |
| `FEAT-led-status-rendering.md` | FX LED 렌더링 | FX 활성화 상태를 LED on/off로 표시한다. |
| `FEAT-display-mailbox-update.md` | FX 상태 mailbox 처리 | 최신 FX 상태 snapshot을 표시 구조에 반영한다. |

### 3.2 REQ-DISPLAY-002 설계

`REQ-DISPLAY-002`는 사용자 입력으로 선택된 패널을 LCD에 출력해야 한다는 요구사항이다.
패널 선택은 UI 상태 머신이 결정하고 상태 관리 구조가 전이를 적용한다.
UI 상태의 생명주기 함수는 render command를 생성하지 않으며, 상태 관리 구조가 이벤트 처리를 마친 뒤 `UI_STATE_RENDER` command를 만들어 표시 구조에 전달한다.

#### 3.2.1 필요 아키텍처

| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-DISPLAY-005` | UI render command | 현재 panel ID와 해당 패널에 바인딩된 모든 파라미터를 한 번에 전달한다. | 자기완결형 `UI_STATE_RENDER`를 사용한다. |
| `ARCH-DISPLAY-006` | display command queue | 패널 전환처럼 누락되면 안 되는 표시 명령을 순서대로 처리한다. | `display_command_queue`를 일반 queue로 둔다. |
| `ARCH-DISPLAY-007` | panel renderer | panel id별 화면 구성을 그린다. | 홈, 트랙, FX, 설정, 진단 패널 renderer를 분리한다. |
| `ARCH-DISPLAY-008` | 패널 파라미터 payload | renderer가 별도 저장소 조회 없이 화면을 완성하게 한다. | panel-parameter binding 순서에 따른 모든 `Parameter` 값을 payload에 복사한다. |

#### 3.2.2 구조 다이어그램

```mermaid
flowchart LR
    State["상태 관리 구조"]
    Queue["display_command_queue"]
    Display["표시 구조"]
    Panel["LCD panel renderer"]
    Lcd["LCD"]

    State -->|UI_STATE_RENDER| Queue --> Display --> Panel --> Lcd
```

#### 3.2.3 동작 시나리오

```mermaid
sequenceDiagram
    participant State as 상태 관리 구조
    participant Display as 표시 구조
    participant Lcd as LCD

    State->>Display: UI_STATE_RENDER(panel_id, bound parameters)
    Display->>Display: panel_id에 맞는 renderer 선택
    Display->>Display: payload의 파라미터로 frame 생성
    Display->>Lcd: 새 패널 화면 출력
```

#### 3.2.4 개발해야 할 기능

| 기능 문서 | 기능 | 목적 |
| --- | --- | --- |
| `FEAT-DISPLAY-001.md` | `UI_STATE_RENDER` payload 정의 | 현재 패널과 바인딩된 모든 파라미터를 표시 command로 표현한다. |
| `FEAT-DISPLAY-002.md` | display command queue 수신 | 패널 전환 command를 누락 없이 queue에 보관한다. |
| `FEAT-DISPLAY-003.md` | display command dequeue | 표시 구조가 패널 전환 command를 순서대로 읽는다. |
| `FEAT-DISPLAY-004.md` | 패널 렌더러 선택 | `panel_id`에 맞는 renderer를 선택한다. |
| `FEAT-DISPLAY-005.md` | 패널 표시 payload 적용 | command에 포함된 panel ID와 파라미터를 현재 렌더링 입력으로 적용한다. |
| `FEAT-DISPLAY-006.md` | LCD 패널 frame 생성 | 선택된 panel renderer가 출력 frame을 만든다. |
| `FEAT-DISPLAY-007.md` | LCD 출력 commit | 생성된 frame을 실제 LCD 출력으로 반영한다. |

### 3.3 REQ-DISPLAY-003 설계

`REQ-DISPLAY-003`은 선택된 디스플레이 패널에 표시될 값이 바뀌면 LCD 값이 갱신되어야 한다는 요구사항이다.
원본 값은 상태 관리 구조가 항상 최신 상태로 유지하지만, 표시 구조가 모든 패널의 파라미터와 설정값을 최신으로 보관할 필요는 없다.
StateTask는 하나의 `StateEvent`에 따른 상태 전이와 파라미터 변경을 모두 완료한 뒤 render 필요 여부를 확인한다.
파라미터가 변경되어 render가 필요하면 현재 선택된 패널이 실제로 출력해야 하는 모든 바인딩 파라미터를 하나의 command에 복사해 표시 구조에 전달한다.
표시 구조는 변경된 파라미터가 출력되는 일부 영역만 갱신하지 않고, 패널 전환 때와 같은 renderer와 출력 파이프라인을 사용해 현재 패널의 전체 frame을 다시 생성하고 LCD에 반영한다.

#### 3.3.1 필요 아키텍처

| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-DISPLAY-009` | 전체 패널 재렌더링 조건 | 파라미터 변경 또는 패널 전이가 발생했을 때 현재 패널의 전체 frame 생성을 요청한다. | 상태 관리 구조가 StateEvent 처리 완료 후 `PARAMETER_UPDATED` 또는 `TRANSITION` 결과 flag를 확인하고 하나의 `UI_STATE_RENDER`를 생성한다. |
| `ARCH-DISPLAY-010` | 전체 패널 표시 payload | 현재 패널의 전체 frame을 그리는 데 필요한 값을 묶어 보낸다. | current panel에 등록된 binding을 순회해 모든 `Parameter` 값을 payload에 복사하고 부분 영역 갱신 정보는 전달하지 않는다. |
| `ARCH-DISPLAY-011` | 패널 전환 초기 payload | 패널이 바뀌면 새 패널의 초기 출력에 필요한 값을 원자적으로 전달한다. | panel ID와 바인딩된 모든 파라미터를 하나의 `UI_STATE_RENDER`에 포함한다. |
| `ARCH-DISPLAY-012` | 표시 구조 최소 상태 | 표시 구조는 command 자체만으로 frame을 생성한다. | canonical 파라미터 저장소에 접근하거나 표시하지 않는 패널의 값을 보관하지 않는다. |
| `ARCH-DISPLAY-013` | telemetry 표시 | 레벨 미터처럼 현재 화면에 항상 노출되는 값만 직접 표시 구조로 보낼 수 있다. | 현재 출력 영역에 없는 telemetry는 표시 갱신 대상이 아니다. |
| `ARCH-DISPLAY-028` | 전체 패널 frame 재생성 | 표시 구조는 파라미터 변경 시 현재 패널의 전체 frame을 다시 만든다. | panel renderer가 frame buffer를 초기화한 뒤 전체 레이아웃과 모든 표시값을 그리고 LCD에 commit한다. |

#### 3.3.2 구조 다이어그램

```mermaid
flowchart LR
    State["상태 관리 구조"]
    Canonical["canonical state"]
    Selector["현재 패널 전체 표시값 선별"]
    Display["표시 구조"]
    Renderer["현재 패널 renderer"]
    Lcd["LCD 전체 frame"]

    Canonical --> State --> Selector
    Selector -->|UI_STATE_RENDER| Display --> Renderer --> Lcd
```

#### 3.3.3 동작 시나리오

```mermaid
sequenceDiagram
    participant State as 상태 관리 구조
    participant Display as 표시 구조
    participant Lcd as LCD

    State->>State: StateEvent의 상태 전이와 파라미터 변경 완료
    State->>State: 처리 결과 flag와 현재 UI panel 확인
    alt 파라미터 변경 또는 패널 전이
        State->>State: current panel binding의 모든 파라미터 복사
        State->>Display: UI_STATE_RENDER(panel ID, bound parameters)
        Display->>Display: panel ID에 맞는 renderer 선택
        Display->>Display: frame buffer 초기화 후 현재 패널 전체 그리기
        Display->>Lcd: 전체 panel frame commit
    else render 불필요
        State->>State: 표시 command 생략
    end
```

#### 3.3.4 개발해야 할 기능

| 기능 문서 | 기능 | 목적 |
| --- | --- | --- |
| `FEAT-DISPLAY-001.md` ~ `FEAT-DISPLAY-007.md` | 공통 패널 출력 파이프라인 | `UI_STATE_RENDER`를 전달받아 현재 패널 renderer로 전체 frame을 생성하고 LCD에 commit한다. |
| `FEAT-DISPLAY-008.md` | 파라미터 변경 시 전체 패널 재렌더링 | 파라미터 변경 결과를 감지해 현재 패널의 전체 frame 생성을 요청한다. |

### 3.4 REQ-DISPLAY-004 설계

`REQ-DISPLAY-004`는 각 트랙 상태를 LED 색상으로 표현해야 한다는 요구사항이다.
트랙 상태는 여러 트랙이 동시에 변할 수 있으므로 단일 트랙 메시지 대신 전체 트랙 상태 snapshot을 전달한다.

#### 3.4.1 필요 아키텍처

| 설계 ID | 아키텍처 항목 | 역할 | 설계 결정 |
| --- | --- | --- | --- |
| `ARCH-DISPLAY-014` | 트랙 상태 snapshot | 모든 트랙 상태와 LED 표시값을 묶어 전달한다. | `TRACK_STATE_SNAPSHOT_RENDER`를 사용한다. |
| `ARCH-DISPLAY-015` | 트랙 상태 mailbox | 최신 트랙 상태 snapshot만 유지한다. | `track_state_mailbox`를 overwritable queue로 둔다. |
| `ARCH-DISPLAY-016` | LED 색상 mapping | 트랙 상태를 LED 색상으로 변환한다. | `IDLE`, `RECORDING`, `PLAYING`, `OVERDUBBING`, `STOPPED` 색상을 매핑한다. |
| `ARCH-DISPLAY-017` | LCD 트랙 상태 표시 | 트랙 패널이 표시 중이면 상태 문자열과 길이를 함께 갱신한다. | snapshot payload의 track list를 사용한다. |

#### 3.4.2 구조 다이어그램

```mermaid
flowchart LR
    State["상태 관리 구조"]
    Mailbox["track_state_mailbox"]
    Display["표시 구조"]
    Led["트랙 LED"]
    Lcd["LCD 트랙 영역"]

    State -->|TRACK_STATE_SNAPSHOT_RENDER| Mailbox --> Display
    Display --> Led
    Display --> Lcd
```

#### 3.4.3 동작 시나리오

```mermaid
sequenceDiagram
    participant State as 상태 관리 구조
    participant Display as 표시 구조
    participant Led as 트랙 LED
    participant Lcd as LCD

    State->>Display: TRACK_STATE_SNAPSHOT_RENDER(all tracks)
    Display->>Display: 트랙별 state를 LED 표시값으로 변환
    Display->>Led: 트랙별 LED 색상 갱신
    opt 현재 패널이 트랙 상태 표시 중
        Display->>Lcd: 트랙 상태 영역 갱신
    end
```

#### 3.4.4 개발해야 할 기능

| 기능 문서 | 기능 | 목적 |
| --- | --- | --- |
| `FEAT-led-status-rendering.md` | 트랙 LED 렌더링 | 트랙 상태를 색상으로 표시한다. |
| `FEAT-lcd-panel-rendering.md` | 트랙 상태 LCD 렌더링 | 트랙 패널에 상태와 길이 정보를 표시한다. |

## 4. 공통 설계 정보

### 4.1 전체 표시 경로

설계 ID: `ARCH-DISPLAY-018`

```mermaid
flowchart LR
    State["상태 관리 구조"]
    Audio["오디오 처리 구조"]
    Queue["display_command_queue"]
    Display["표시 구조"]
    Lcd["LCD"]
    Led["LED"]

    State -->|UI_STATE_RENDER: panel ID와 bound parameters| Queue --> Display
    Audio -->|현재 출력 영역 telemetry| Display
    Display --> Lcd
    Display --> Led
```

### 4.2 Queue와 mailbox

| 설계 ID | 채널 | 수신 방식 | 사용 기준 |
| --- | --- | --- | --- |
| `ARCH-DISPLAY-019` | `display_command_queue` | 일반 queue | panel ID와 바인딩 파라미터를 포함한 `UI_STATE_RENDER`, 초기화와 밝기 변경처럼 순서가 중요한 명령 |
| `ARCH-DISPLAY-020` | `track_state_mailbox` | overwritable mailbox | 모든 트랙의 최신 상태 snapshot |
| `ARCH-DISPLAY-021` | `track_param_mailbox` | 대체됨 | `UI_STATE_RENDER`의 bound parameters로 대체되어 구현하지 않는다. |
| `ARCH-DISPLAY-022` | `fx_state_mailbox` | overwritable mailbox | IFX/TFX 선택과 활성화 상태 snapshot |
| `ARCH-DISPLAY-023` | `fx_param_mailbox` | 대체됨 | `UI_STATE_RENDER`의 bound parameters로 대체되어 구현하지 않는다. |
| `ARCH-DISPLAY-024` | `system_mailbox` | 대체됨 | `UI_STATE_RENDER`의 bound parameters로 대체되어 구현하지 않는다. |
| `ARCH-DISPLAY-025` | `diagnostic_mailbox` | 대체됨 | 일반 UI 파라미터는 `UI_STATE_RENDER`로 전달하고, 비동기 진단 telemetry가 필요할 때 별도 설계를 검토한다. |
| `ARCH-DISPLAY-026` | `track_position_mailbox` | overwritable mailbox | 현재 패널의 진행률 영역에 표시할 트랙 위치 telemetry |
| `ARCH-DISPLAY-027` | `level_meter_mailbox` | overwritable mailbox | 현재 출력 영역에 표시할 입력/출력 레벨 telemetry |

검토 결과 `track_param_mailbox`, `fx_param_mailbox`, `system_mailbox`와 일반 파라미터용 `diagnostic_mailbox`는 자기완결형 `UI_STATE_RENDER`와 역할이 중복되므로 구현 대상에서 제외한다.
관련 설계 ID와 기존 기능 문서 항목은 삭제하지 않고 대체 사실을 기록한다.
트랙 상태 LED snapshot과 화면 갱신 주기가 StateEvent와 다른 실시간 telemetry만 overwritable mailbox 대상으로 유지한다.

### 4.3 LED 표시 기준

| 대상 | 상태 | 표시 |
| --- | --- | --- |
| FX | 활성화 | on |
| FX | 비활성화 | off |
| 트랙 | `IDLE` | off |
| 트랙 | `RECORDING` | 빨간색 |
| 트랙 | `PLAYING` | 초록색 |
| 트랙 | `OVERDUBBING` | 노란색 |
| 트랙 | `STOPPED` | 파란색 |

## 5. 기능 문서 작성 대상

| 기능 문서 | 목적 | 주요 입력 | 주요 출력 |
| --- | --- | --- | --- |
| `FEAT-DISPLAY-001.md` | 현재 panel ID와 바인딩된 모든 파라미터를 `UI_STATE_RENDER` payload로 정의한다. | panel ID, bound parameters | `UiStateRenderPayload` |
| `FEAT-DISPLAY-002.md` | UI render command를 `display_command_queue`에 보관한다. | `UI_STATE_RENDER` | queued display command |
| `FEAT-DISPLAY-003.md` | 표시 구조가 display command를 순서대로 읽는다. | `display_command_queue` | display command dispatch request |
| `FEAT-DISPLAY-004.md` | `panel_id`에 맞는 LCD panel renderer를 선택한다. | panel ID, bound parameters | panel render request |
| `FEAT-DISPLAY-005.md` | command payload를 별도 canonical context 없이 renderer 입력으로 적용한다. | `UiStateRenderPayload` | renderer input |
| `FEAT-DISPLAY-006.md` | 바인딩 파라미터로 선택된 패널의 LCD frame을 생성한다. | panel render request | LCD frame buffer |
| `FEAT-DISPLAY-007.md` | LCD frame을 실제 출력으로 반영한다. | LCD frame buffer | LCD output |
| `FEAT-DISPLAY-008.md` | 파라미터 변경 시 현재 패널의 전체 frame 재생성을 요청한다. | parameter updated result, current panel | `UI_STATE_RENDER` |
| `FEAT-display-mailbox-update.md` | 패널 파라미터 mailbox 기능은 `UI_STATE_RENDER`로 대체되어 작성 대상에서 제외한다. | - | 대체 기록 |
| `FEAT-lcd-panel-rendering.md` | 선택된 LCD 패널과 전달받은 표시값을 그린다. | panel id, display payload | LCD output |
| `FEAT-led-status-rendering.md` | FX와 트랙 LED 상태를 갱신한다. | FX/track state snapshot | LED output |
| `FEAT-level-meter-rendering.md` | 오디오 레벨 telemetry를 레벨 미터로 표시한다. | level meter payload | LCD meter output |

## 6. 미정 사항

| 항목 | 결정 필요 내용 | 영향 |
| --- | --- | --- |
| LED 실제 색상 출력 방식 | LED 종류와 MCP23017 pin mapping을 확정해야 한다. | LED renderer |
| mailbox poll 주기 | 표시 태스크가 mailbox를 확인하는 주기를 정해야 한다. | 표시 응답성 |
| 레벨 미터 scaling | audio level 값을 화면 높이로 변환하는 기준을 정해야 한다. | 사용자 피드백 품질 |
