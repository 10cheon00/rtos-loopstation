...
구현 일지는 `/dev-log/`하위에 저장되어 있습니다. 각 문서는 일 단위로 작업한 내용을 담고 있습니다.
...
일단 녹음이 완료되면 녹음된 데이터는 처음부터 재생되며 끝까지 재생된 경우 다시 처음으로 돌아가 반복 재생합니다. 

각 버튼의 자세한 동작은 [트랙의 상태](#트랙의-상태)절에 명시되어 있습니다.

정지 버튼을 누르면 재생중인 오디오 데이터의 출력이 정지됩니다. 

트랙 설정 버튼을 누르면 트랙의 여러가지 설정들을 수정하는 패널로 이동하게 됩니다. 
...
위 5가지 상태 다이어그램은 [software_design.md##트랙 상태 머신](/docs/software_design.md##트랙-상태-머신)에 정리되어 있습니다..
...
> 각 FX에 대한 설명과 파라미터는 [fx_design.md](../fx_design.md)를 기준으로 정리합니다.
...
디스플레이 화면은 계층형 패널 구조로 구성됩니다. 좌우 버튼을 눌러 같은 계층의 다른 패널로 전환하거나, Enter, Exit버튼을 눌러 하위 패널로 진입 또는 상위 패널로 이동할 수 있습니다. 패널 순서와 트리 구조는 [software_design.md##패널 계층도](/docs/software_design.md##패널-계층도)에 정리되어 있습니다.
...
- 트랙 상태 패널(1~트랙 수만큼 존재)

    - `트랙 상태` : 선택된 트랙이 가지는 상태를 문자로 보여줍니다.
    - `트랙 볼륨` : 선택된 트랙의 볼륨 크기를 보여줍니다.
...
LED는 디스플레이에 표시되는 정보들 중 일부분을 나타냅니다. LED를 통해 상태를 직관적으로 나타내야하는 정보들은 다음과 같습니다.
